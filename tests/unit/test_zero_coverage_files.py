...
class _FakeStorage:
    ...
    # --- Scalar indexing (v1.7.1) ---
    def create_scalar_index(self, name: str, column: str, **kwargs: Any) -> None: ...
    def create_facet_indexes(
        self, name: str, columns: list[str] | None = None, **kwargs: Any
    ) -> dict[str, str]: ...
...
class TestStorageProtocol:
    ...
    def test_mock_without_spec_passes(self) -> None:
        """A bare MagicMock auto-creates all attributes, so runtime_checkable
        isinstance passes. This documents the known limitation."""
        mock = MagicMock()
        # MagicMock auto-creates attributes matching any protocol method name
        assert isinstance(mock, StorageProtocol)
