...
| # | 能力 | 现状 | 建议 |
|---|---|---|---|
| 5 | ✅ **Reranker(cross-encoder 精排)** | RAG 有 `rag/reranker.py` 完整体系,现已接入 hybrid search | **已完成**(feat/v1.8.0-reranker):`HybridSearchConfig.reranker_type/model` + `HybridSearchBridge._rerank_table`(行→ContextChunk→rerank→take+`_rerank_score`,缺 text 列/异常优雅降级) |
| 6 | ✅ **多模态嵌入(CLIP / imagebind)** | CLIP 图像嵌入已实现（`embed/image_encoder.py` `CLIPImageEncoder.encode`）；vector.search 已支持 `vector_column` | **已完成**(master)：新增 `encode_text`(text tower)补全跨模态 text→image，cookbook `05` STEP 4 演示。注：blob 存储 Lance 原生支持，imagebind/VLM 留 #18 |
...
| # | 能力 | 现状 | 建议 |
|---|---|---|---|
| 9 | ✅ **DuckLake 物化视图(跨存储 join)** | `materialize()` / `cleanup_materialized()` 已在 `olap.py`（经 `DuckLakeWorkspace` TTL + ART index + 行预算） | **已完成**(master，核实)：非 `federated_engine.py`（doc 误标）；29 tests 绿。跨 HugeGraph 元数据物化 join 留后续 |
| 10 | ✅ **SQL-PGQ 属性图查询**(DuckDB 1.5) | 图查询原全走 HugeGraph | **已完成**(master)：PGQ 在此 build 不可用（`pgq` 装不了，ParserException），改用 `OlapSearchBridge.graph_query` 递归 CTE（环安全 BFS 邻居/路径，directed/undirected + 权重），与 HugeGraph 互补；11 tests |
| 11 | ✅ **Prepared statements** | DuckLake 元数据表 INSERT/SELECT/DELETE 已全用 `$1..$4` 参数化 | **已完成**(核实)：DuckDB `EXECUTE` 不支持绑定参数，参数化执行是正确方案（DuckDB 自动缓存计划）；4 tests 守卫不变量 |
...
| # | 建议 | 说明 |
|---|---|---|
| 17 | **全链路 async** | v1.7.1 加了 `search_async`(向量),但 FTS/Hybrid/faceted 仍 sync。**进展(v1.8.x)**:向量 `search_async` 已池化(`async_conn_pool.py`,#1)+修复 `AsyncTable.search` 缺 `await` 的 bug,真正可用;FTS/Hybrid/faceted 原生 async 迁移中(#8);压测待做 |
