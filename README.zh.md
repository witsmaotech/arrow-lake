...
稳定且已在生产中使用。当前版本：**v1.11.6.6** —— **四维 review 加固批（3 HIGH + M1-M12 + LOW 顺手清）**：场景孤儿回收加年龄门槛（V027 `updated_at` + runner 20s 心跳——sibling worker 重启不再误杀活 runner）；resume/middleware 按实例 `scenario_version` 钉版（场景升版不再混入续跑语义）；列级 ACL 封堵两处旁路面（PII suggest 与 `visible_columns` 交集+行级过滤声明；场景实例列表/详情按调用者裁剪 `context.target`、不再回传原始 `context_json`）；M 批——drop_index 后失效池化句柄、dry-run preview 过危险关键字黑名单、PII 单值截断 4096 字符（email 正则按 RFC 上界线性化，扫描 15s→亚秒）、docling page_batch 移到 standard/VLM 汇合点、parse 缓存 key 加 docling 签名、图片描述并发/超时 config 化、AND 批补偿待办聚合、补偿核销端点（+console 按钮）、instantiate 查重 + CAS resume（防双活 runner）、网关降权口径收敛、契约 identifier/person 字段语义作 LOW 证据、实例列表真翻页（offset+COUNT）；LOW 顺手清——lint 剥字符串字面量、isdecimal 维度解析、CAS terminate（不覆写终态）、onnxruntime providers 可观测日志 + 当前版本：**v1.11.6.5** —— **PII 分级自动建议（补丁，复活搁置 W2 #3）**：`GET /datasets/{name}/classification/suggest` 确定性零模型内容扫描——身份证（GB11643 校验位）/手机号/银行卡（Luhn，标注启发式）/邮箱/车牌/详细地址正则 + 列名语义（地址/坐标/联系人；org/uid 负面清单）+ 自由文本提示——输出四档建议与逐列可解释证据（命中率+脱敏样本）；Lance scanner 500 行采样使 107M 行表亚秒级；console 分级面板新增「建议→一键采纳」；建议永不自动写入（登记不校验原则不变）；GLiNER/docling 模型面按产品锚准则裁掉 + 当前版本：**v1.11.6.4** —— **图片描述 enrichment（补丁）**：`docling_picture_description` + endpoint/model/prompt 配置字段，为文档内图片注入中文描述段落（markdown `<!-- image -->` + 描述文本），随解析文本链自动进入 FTS/向量检索——图片内容可查，与 ColPali 向量路线互补；API 型（OpenAI 兼容，key 复用 VLM 字段），默认关，endpoint/model 缺失时 warning 降级不炸转换 + 当前版本：**v1.11.6.3** —— **Docling VLM 档 API 化（补丁）**：VLM 档（`docling_pipeline_type=vlm`）新增四个配置字段（`docling_vlm_endpoint`/`model`/`api_key`/`concurrency`）即可指向任意 OpenAI 兼容 chat-completions 端点——云 API（百炼 qwen-vl-max 已 live 验证：扫描件单页 5.2s vs inline 本地 53.9s，零本地显存）或自托管推理服务（vLLM/Ollama）皆可，inline 本地 Transformers 仍为零依赖默认。平台自身不附带推理服务（定位：数据湖仓，非大模型原生应用）；converter 缓存签名按端点/模型分桶 + 当前版本：**v1.11.6.2** —— **RapidOCR OCR 上 GPU（补丁）**：onnxruntime（CPU）换 onnxruntime-gpu（CUDA 13 构建，与镜像 torch cu130 同代）——CUDA 运行时库经一条 `LD_LIBRARY_PATH` 复用 torch 自带的 `nvidia/cu13` + cudnn，零额外 NVIDIA 包；模型零改动（包内 PP-OCRv6 ONNX），docling 按 accelerator 配置自动开 `use_cuda`（`GPU_COUNT=0` 自动回落 CPU）；OCR 张量从 RAM 挪 VRAM，解历史 page_batch-64 OOM 根因、打开页批上调空间 + 当前版本：**v1.11.6.1** —— **Docling VLM 档修复 + 标题层级恢复（补丁）**：HF_HOME 由只读镜像路径改指可写 docling-cache 卷——standard 双模型与镜像 bake 同 commit 入卷，granite-docling-258M 经 ModelScope 直连（HF 与代理双断时国内镜像同 repo、sha256 校验）以标准 HF cache 布局预置，离线门禁保持；`docling_pipeline_type=vlm` 从空接线转真实可用（live 实测：granite GPU 转换 markdown 结构正确，峰值显存 +1.3GB）；PDF 标题层级恢复（书签/编号/字体样式三信号，`docling_heading_hierarchy` 默认开、零模型开销——markdown 深度、分块 heading path、RAG 结构信号不再扁平 level-1）；dev override HF 布线与生产对齐 + 当前版本：**v1.11.6** —— **字段与多模态类型扩展**：schema/migrate dry_run 方言 lint（TRIM/SUBSTR/双引号字面量/CASE 四坑静态拦截）+ 表达式 5 行采样试算；加列双模式（SQL 表达式 ↔ 类型化占位列）——vector/blob/timestamp/date32/large-* 列经预计算列全 NULL 落库、后回填（向量端到端验证：占位 → embed 空值回填 → 语义检索命中）；blob 列按数据文件版本 ≥2.2 门控并给出重写/内嵌 binary 指引（lancedb 默认写 2.1）；未知类型全路径 fail-closed；向量维度检查激活；string→binary 语义防护 + 当前版本：**v1.11.5** —— **投产准备批 + 场景执行引擎（MS3 从规范转正执行面）：conftest 测试隔离四锚点（storage/api-key/rate-limit/redis 按会话钉死——「全量 448 随机失败」类根除，全量套件 448 failed→2 个第三方失败/64min 稳定）+ CI 骨架三 job（lint E9/F821 门禁 / 全量单测 / runtime-ci 镜像 build+容器冒烟）；场景执行引擎（`actions/runner.py`：XOR 网关在引用步 requires 处求值且落选臂 skipped 级联、AND 汇聚并发、死锁守卫、deadline 超时升级、声明式补偿挂人工待办而非自动执行、terminate、崩溃断点续跑尊重幂等 already-in-effect）+ V025 场景实例两表 + 五端点，console actions.html 升级为执行态视图；数据集 PII 分级（V026 四档，登记不校验）+ 分级↔脱敏绑定校验（未分级 corpus 导出 422 须显式豁免，留审计）；confidence 真实语义（未命中规则 0.5 基线、每条 unruly 规则扣分封顶 −0.3、substitute 臂 ×0.9——不再恒 1.0，飞轮低置信回流有真实筛选力）；以及作为本版发版闸门的真实数据首跑：合肥液化气隐患 2 万行全链贯通——契约 v2（quality 按域调权，近恒定枚举 diversity 归零）、confidential 分级、双标注 κ=1.0、五维 98.69/gold、v1.1.0 干净发布不走 force——首跑当日清 4 真缺陷（标注采样稀疏列饥饿、cap 先截断后过滤、LS 标准三分类相关性词表、悬空 prediction relation 崩标注页）** + 当前版本：**v1.11.4** —— **MS5 五维质量门+发布层（Master Plan 收官）：五维评估引擎（契约声明权重/星级/bronze-silver-gold 准入/逐维一票否决与诚实降级）、相关性回路（LS 三分类标注 + LLM 直评降级）、漂移监控（KL 散度 + 发布时基线快照）、发布门（准入+否决+拒绝劣化+漂移，force 留审计，Lance 版本锁定 + 语义化 tag + 规格书生成）、语料四形态（ADL→SFT、KG 快照→预训练、研判历史×ADL→RLHF 配对、黄金集——全部经红线④脱敏 fail-closed 出域）、数据飞轮（低置信回流 + 相关性反哺清单）、F1.7 专家评审材料包；同日四维综合 review（质量/性能/测试/安全，1 CRITICAL+15 HIGH+22 MEDIUM）五批全清——第二标注者回收治本（复合 watermark + webhook 触发回收）、跨进程回收互斥、event-loop 解冻（drift/corpus 向量化）、行动写唯一性核验、治理审计补齐、332 测试解封；另全站 console UI 实测（23 页 64 项交互浏览器逐一验证，修复 8 处从未通过过的 API 调用）、生产向导页、本体建模工作台、导航分组折叠）** + 当前版本：**v1.11.3** —— **MS4 标注闭环（旁路模式标注供应链，热路径零改动）：Label Studio CE 1.13.1 以独立 compose profile 作为 transient 工作区（AL 侧 Lance ADL 为唯一真值源）；本体模板 → LS 标注界面生成器（实体/关系 + L4 五段，支持手写 XML 覆盖）；零依赖 LSClient（legacy Token 直连 + PAT refresh 回退，内网显式零代理直连）；标注项目注册表（V018 + V019 watermark）；四策略采样（uncertainty/diversity/failure_case/committee，最大余数法配额 + 数据源缺失自动重分配）；预标注 = 真 hyper-extract 抽取以内嵌静态 predictions 进 LS（不建 ML backend）；派发前脱敏（L2 泛化 + L3 HMAC 假名，fail-closed，抽取跑在脱敏文本上保证 span 对齐）；一键 dispatch（202 后台）；回收走 export 全量对账（adl_id 内容哈希幂等 + (row,annotator) 版本链 + 仅随带标注任务推进的 watermark）；Fleiss' kappa + signature 仲裁状态机（分歧自动生成仲裁 task，ground truth 免检闭环）；30s 回收 scheduler（熔断）；console 标注管理页；W4 fan-out review 13 项 + W5 live 校准 6 项（row_id 稳定化、404-only 重绑、async loop 亲和、require_masking 门、webhook 密钥、scoped MinIO 凭据）全部当日清偿；试点以真百炼 × 真 LS 跑通全流程，kappa/仲裁/版本链/审计 DoD 断言全过）** + 当前版本：**v1.11.2** —— **MS3 决策与行动层（手写 tokenizer + frozen AST 的谓词 DSL、V016/V017 行动目录与场景版本链、幂等先于前置的八步执行中间件、研判 API、console 页与 vertical-slice e2e）** + 当前版本：**v1.11.1** —— **MS2 语义层（对象标识 + 语义对齐 + Object Set 对象查询）：DR15 D-1/D-2 模型扩展——契约新增列 `label`／表节 `lifecycle`／引用 `cardinality`+`kind`（登记不校验，compiler 输出零变化），ontology_rules 新增五分类 `rule_type` + 独立 `version`（V013）；F2.1 对象标识（契约 pattern 标识解析 + V014 entity_map 源系统 ID→对象 ID）；F2.2 语义对齐（含温度偏移的仿射单位注册表、%LEL 独立维度；封闭种类配置 V015 + lineage 事件；查询视图 SQL 投影）；F2.3 `POST /api/v1/objects/query` 返回业务对象（标识组件／label+unit 属性／lifecycle 状态／带基数的 `_links`／可选 KG 邻居／active 规则引用），服务端拼装受限 SQL 走 /query/olap 同一条安全路径；F2.4 console 对象浏览页；W1–W4 三路 review（0 critical）当日清偿；跨源同口径 DoD 固化为自动化 e2e（同一物理量经两源系统→同一对象、同一属性、同一单位）** + 当前版本：**v1.11.0.3** —— **加固批（契约 enforce 试点 + 首页 401 根治 + review P2 六项）：per-dataset 质量门禁档覆盖（`GATE_MODE_OVERRIDES` JSON blob，demo_gas 经基线门控 0.00%/<10% + 零 skipped 切 enforce，契约拒收行死信带具体 violation 明细）；浏览器 favicon 401 根治（根路径公开 + 资产 + 长缓存——"首页间歇性 401"探针的正源）；review §三 P2 六项清偿（容器表删除回收 registry 行 + DELETE ?table=、list_container_tables 探测不再 mkdir 垃圾目录、容器级删除与表级写锁互斥、Gravitino per-table 4xx 不再中断同容器其余表、契约 pattern 内层 regex 保存时试编译 + 长度帽、基线 NULL 样本显式 `<NULL>`）** + **v1.11.0.2** —— **数据集契约 + 多表容器（DR13/DR14）：契约包把每表列规则编译为 DuckDB 谓词，作为质量门禁第四 stage 接线（shadow/enforce，enforce 下评估异常 fail-closed）；SQL/ClickHouse/Kafka/Iceberg/Delta 五类源全部过门禁；契约管理 API + console 契约页 + 带 --max-rows 护栏的离线基线脚本；多表容器数据集以 `?table=` 二段名寻址（{name} 路由 pattern 继续拒绝点号）、Gravitino dataset→schema 映射、容器化备份/恢复与 console 容器视图 —— 发版前四维 review（P0×9 + P1×10）当日全部清零：表级 deny-read 端到端闭环、池化 DuckDB 会话每次查询后清理（跨用户注册读封堵）、门禁 fail-open 路径全除** + **v1.11.0** —— **MS1 本体与规则地基（模板隐式本体形式化为机器可校验的 SHACL —— 模板 `ontology:` 段，旗舰模板 project_concept_graph 落地 22 实体类/16 关系/72 类型配对并与运行时白名单同源钉死；KG 构建收尾挂本体门禁，shadow 模式默认、reject/warn 分级、超时 fail-closed、`arrow_lake_ontology_check_total` 指标与构建状态违规摘要；system_db V010 本体版本链 + 结构化 diff + 可审计规则注册表；`/api/v1/ontology` 管理接口与 console 本体页；本体→guideline 规则渲染让抽取 prompt 与校验器同源；旗舰数据集缺陷逃逸基线 29.21% —— enforce 维持关闭）** + **v1.10.7** —— **v1.10.7 P1 数据面加固（sqlglot 源头级数据集 ACL 强制——行过滤下推进查询、隐藏列引用在 AST 层拒绝，封死别名/聚合绕过类；全部读端点挂载 deny/ACL 守卫；RAG 对 ACL 受限数据集 fail-closed；ingest 工作隔离到专属线程池；认证热路径移出事件循环；SIGTERM 不再被劫持、优雅停机真正执行；摄入质量门控以 shadow 模式接线并修复三个潜伏 bug；连接池/tag-ACL/embed 守卫可靠性修复）**，以及收敛进 v1.10.7 的发布后四维 review 加固（SQL ACL 大小写不敏感匹配——CRITICAL 绕过——加 COLUMNS() 通配拒绝、作用域感知别名解析、含 `--` 名的行过滤子查询加引号、全拒批门控修复、tag-ACL 回收 fail-open 封堵）+ **v1.10.6 认证/授权原生加固（JWT `aud` claim + per-user token 失效、admin 签发的一次性密码重置、admin 全写操作审计 + 共享 API key 弃用引导、JWKS 端点 + RS256 推荐、`require_permission` scope 化鉴权（空 permissions 回退 role 兼容）—— 未来接外部 IdP 的四条接缝全部就位而无需现在引入）** + v1.10.0 知识抽取模板管理（M1–M5：动态加载、CRUD、绑定、AI 编写、试运行、质量验证 harness）+ v1.10.1 稳定性与治理加固（docling GPU triton JIT 修复、KG 模板降级路径、配置精简、examples 整合进 cookbook）+ **v1.10.2 文本文档增量构建（ingest/KA/KG 增量）、性能基准套件扩充、超时/可靠性加固（OLAP `conn.interrupt()` 看门狗避免卡住扫描拖死连接池、异步任务心跳 + 僵尸回收避免 worker 死亡后任务永久卡 running、ingest 全链路补 per-call timeout），并复测 SSD 性能基线** + v1.10.3 docling 吞吐与质量优化（ThreadedPdfPipelineOptions 页批处理跑满 GPU、RapidOCR、置信度门控 OCR 重试、页面图片导出支持 ColPali/CLIP 多模态 RAG、bake bge-m3 tokenizer 让 HybridChunker 离线生效）+ **v1.10.4 per-dataset native lance scan opt-in + D-state 熔断器（无向量大数据集经 Rust 聚合下推快 34–145×，熔断器在重复 D-state 时自动降回 pyarrow 冷却）、OLAP 结果分页 + 字段分布统计明细、多语句 SQL 防护、structlog 级别过滤 + gravitino 同步周期日志降噪**。完整历史与路线图方向（更深入的多模态、分布式扩展、更多抽取后端）见 [CHANGELOG](CHANGELOG.md)。
