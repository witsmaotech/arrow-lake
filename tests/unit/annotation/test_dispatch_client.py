...
class TestAuth:
    def test_legacy_token_header_direct(self):
        """Token 头直连(W5 live 语义:refresh 端点在 LS 1.13.1 部署 404)。"""
        opener = FakeOpener([
            ("GET", "/api/projects?page_size=100", 200, [{"id": 7}]),
        ])
        client = _client(opener)
        assert client.list_projects() == [{"id": 7}]
        assert len(opener.calls) == 1  # 不 refresh
        assert opener.calls[0]["headers"]["authorization"] == "Token pat-token"

    def test_401_falls_back_to_refresh_then_bearer(self):
        opener = FakeOpener([
            ("GET", "/api/projects?page_size=100", 401, {"detail": "no"}),
    ...
    def test_bearer_cached_across_calls(self):
        opener = FakeOpener([
            ("GET", "/api/projects?page_size=100", 401, {}),
            ("POST", "/api/token/refresh", 200, {"access": "acc-1"}),
            ("GET", "/api/projects?page_size=100", 200, []),
            ("GET", "/api/projects?page_size=100", 200, []),
        ])
        client = _client(opener)
...
class TestTasks:
    def test_import_tasks_posts_to_project_import(self):
        ...
        assert json.loads(opener.calls[0]["body"]) == tasks
