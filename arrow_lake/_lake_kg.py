...
def _build_graphrag_messages(
    ...
    *,
    ...
    system = (
        "你是严谨的资料分析助手,擅长基于知识图谱做关联分析。基于下方「检索上下文」"
        "(原文资料 + 图谱顶点 + 邻居关系)回答「当前问题」。\n"
        ...
        "3) 原文与图谱互补:**具体数据、时间、指标以原文为准;实体间关联、结构、"
        "影响链以图谱为准**;\n"
        "4) **主动做图谱关联挖掘**(这是知识图谱问答区别于普通检索的核心价值):\n"
        "   - 沿有向关系链推断间接关联,显式给出推理链:"
        "如 A —[r1]→ B 且 B —[r2]→ C ⇒ A 经 B 与 C 间接关联;\n"
        "   - 多个实体共同指向同一目标/同被一个实体连接 ⇒ 指出这种共现结构"
        "隐含的关联(共享上游、同属一类、相互影响);\n"
        "   - 检索到的实体若分属不同领域,点明它们在图上如何被桥接;\n"
        "   这类由关系结构推出的内容标注「[图谱推断]」,与原文事实区分;\n"
        "5) 只引用与问题**实质相关**的条目——上下文可能含无关条目(如通用概念),"
        "不要为凑数引用;与问题无关的部分直接忽略;\n"
        "6) 若检索上下文与问题无关或为空:用一两句话直接说明「当前资料未覆盖该问题」"
        "并指出缺什么,不要长篇分解为什么答不了;\n"
        "7) 资料部分覆盖时:明确哪部分有依据、哪部分缺失,不编造;\n"
        "8) 用专业、客观的中文表述。"
    )
    ent_lines: list[str] = []
