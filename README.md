...
Multimodal storage · Vector & full-text search · SQL analytics · Knowledge Graph · GraphRAG · Document AI —
all running over **the same datasets, under one governance plane**. One self-hosted platform, not five tools stitched together.

[![Version](https://img.shields.io/badge/version-1.11.6.6-blue?style=flat-square)](#)
...
Stable and in production use. Current release: **v1.11.6.6** — **Review-hardening patch (four-dimension review: 3 HIGH + M1-M12 + LOW sweep)**: scenario orphan reaping is age-gated (V027 `updated_at` + runner heartbeat — a sibling worker restart no longer kills live runners); resume/middleware pin the instance's `scenario_version` (an upgraded spec never bleeds into a resumed run); column-level ACL closed on both bypass read faces (PII suggest intersects `visible_columns` with a row-filter disclosure note; scenario instance list/detail prune `context.target` per caller and stop returning raw `context_json`); plus the M-batch — drop-index pool-handle invalidation, dry-run preview runs the dangerous-keyword blacklist, PII per-value truncation at 4096 chars with a linear-time email regex, docling `page_batch` applied at the standard/VLM convergence point, parse cache keyed by the docling signature, picture-description concurrency/timeout now config fields, AND-batch compensation aggregation, a per-step compensation ack endpoint (+console button), instantiate dedup + CAS resume (no dual live runners), contract identifier/person field semantics as LOW evidence, instance-list pagination with true totals; LOW sweep: lint strips string literals, `isdecimal` dimension parsing, CAS terminate (no terminal-state overwrite), onnxruntime provider observability log + Current release: **v1.11.6.5** — **PII classification auto-suggest (patch, revives shelved W2 #3)**: `GET /datasets/{name}/classification/suggest` runs a deterministic, model-free content scan — ID-card (GB11643 checksum) / mobile / bank-card (Luhn, labeled heuristic) / email / plate / street-address regexes + column-name semantics (address/geo/contact; org/uid negative list) + free-text hints — and suggests one of the four tiers with explainable per-column evidence (hit ratio + masked samples). 500-row Lance-scanner sampling keeps 107M-row tables at sub-second; the console classification panel gains suggest→one-click-adopt; suggestions never auto-write (registration-stays-unvalidated principle unchanged). The GLiNER/docling model face was deliberately cut per the product-anchor rule + Current release: **v1.11.6.4** — **Picture-description enrichment (patch)**: `docling_picture_description` + endpoint/model/prompt config fields inject Chinese image captions as markdown paragraphs (via `<!-- image -->` + description text) straight into the parsed text chain — so images become searchable by FTS/vector retrieval, complementing the ColPali embedding route; API-based (OpenAI-compatible, key shared with the VLM field), off by default, missing endpoint/model degrades to a warning without breaking conversion + Current release: **v1.11.6.3** — **Docling VLM pipeline over OpenAI-compatible APIs (patch)**: the VLM tier (`docling_pipeline_type=vlm`) can now target any OpenAI-compatible chat-completions endpoint via four config fields (`docling_vlm_endpoint` / `model` / `api_key` / `concurrency`) — cloud APIs (Bailian qwen-vl-max verified live: scanned page 5.2s vs 53.9s inline, zero local VRAM) or self-hosted inference services (vLLM/Ollama) — with inline local Transformers remaining the zero-dependency default. The platform ships no inference service of its own (positioning: a data lakehouse, not a model-native app); converter cache signatures bucket by endpoint/model + Current release: **v1.11.6.2** — **RapidOCR OCR on GPU (patch)**: onnxruntime (CPU) swapped for onnxruntime-gpu (CUDA 13 build, same generation as the image's torch cu130) — CUDA runtime libs reused from torch's bundled `nvidia/cu13` + cudnn via one `LD_LIBRARY_PATH`, no extra NVIDIA packages; models unchanged (in-package PP-OCRv6 ONNX), docling auto-enables `use_cuda` from its accelerator config (CPU fallback when `GPU_COUNT=0`), unlocking headroom to raise the docling page batch (OCR tensors move from RAM to VRAM — the root cause of the historical page_batch-64 OOM) + Current release: **v1.11.6.1** — **Docling VLM pipeline fix + heading-level restoration (patch)**: HF_HOME moved from the read-only image path to the writable docling-cache volume — standard-pipeline models staged at the same commits as the image bake, plus granite-docling-258M pulled over ModelScope direct (HF and the proxy were both down; same repo, sha256-verified) into a hand-built HF cache layout, offline gate kept; `docling_pipeline_type=vlm` now actually works (live: granite on GPU produces correct markdown, +1.3 GB VRAM peak); PDF heading levels restored from bookmarks/numbering/font-style signals (`docling_heading_hierarchy`, on by default, zero model cost) so markdown depth, chunk heading paths, and RAG structural signals stop being flat level-1; dev-override HF wiring aligned with production + Current release: **v1.11.6** — **field & multimodal schema evolution**: schema/migrate dry-run dialect lint (TRIM/SUBSTR/double-quote-literal/CASE traps) + 5-row expression sampling; typed-placeholder add mode — vector/blob/timestamp/date32/large-* columns land all-NULL via precomputed columns and fill via backfill (vector end-to-end verified: placeholder → embed null-backfill → semantic search hit); blob columns gated on data-file version >= 2.2 with rewrite/binary guidance (lancedb defaults to 2.1); unknown types fail closed on every path; vector dimension checks activated; string→binary semantic guard + Current release: **v1.11.5** — **production-readiness batch + scenario execution engine (MS3 promoted from spec to runtime): test-isolation anchors in conftest (storage / api-key / rate-limit / redis env pinned per session — the "448 random failures" class rooted out, full suite now 448-failed→2-third-party in 64min) plus a three-job CI skeleton (lint E9/F821 gate / full unit tests / runtime-ci image build + container smoke); the scenario runner (`actions/runner.py`: XOR gateways evaluated at referencing-step requires with skipped-arm cascading, AND-join concurrency, deadlock guard, deadline escalation, declarative compensation into human to-dos rather than auto-execution, terminate, and crash-resume honoring idempotent already-in-effect steps) on V025 instance tables with five endpoints, console actions.html upgraded to runtime execution views; dataset PII classification (V026 four tiers, registry-not-gate) with classification↔masking binding checks (unclassified corpus exports 422 unless explicitly opted in, audited); confidence with real semantics (0.5 baseline when no rule hits, per-unruly-rule penalties capped at −0.3, substitute-arm ×0.9 — no longer constant 1.0, so the flywheel's low-confidence requeue actually filters); and the real-data first run that gated this release: 20k rows of Hefei LPG-hazard inspections through the full chain — contract v2 with domain-tuned quality weights (diversity zeroed for near-constant enums), confidential classification, dual annotation κ=1.0, five-dimension 98.69/gold, and a clean v1.1.0 release without force — with four first-run defects cleared same-day (sparse-column sampling starvation, cap-before-filter truncation, the LS standard three-way relevance vocabulary, and dangling prediction relations crashing the LS labeling page)** + Current release: **v1.11.4** — **MS5 five-dimension quality gate + release layer (the Master Plan finale): a five-dimension assessment engine (contract-declared weights, star ratings, bronze/silver/gold admission, per-dimension one-vote vetoes with honest degradation), a relevance loop over Label Studio three-way classification (LLM-direct-eval fallback), drift monitoring via KL divergence with release-time baseline snapshots, a release gate (admission + veto + regression-rejection + drift, audited force override, Lance version lock + semver tags + generated datasheets), four corpus export forms (SFT from ADL, pretraining from KG snapshots, RLHF pairs from decision history x ADL, golden sets — all masked out-of-domain under the fail-closed red line), the data flywheel (low-confidence requeue + relevance feedback lists), and an F1.7 expert review pack; same-day four-dimension review (quality/performance/test/security, 1 CRITICAL + 15 HIGH + 22 MEDIUM) cleared in five batches — the second-annotator recovery fix (composite watermark + webhook-triggered recover), cross-process recovery locking, event-loop unblocking with vectorized drift/corpus paths, action write uniqueness verification, governance audit coverage, and a 332-test unblock; plus a full-site console UI pass (23 pages, 64 interactions all browser-verified, 8 never-worked API call sites fixed), a production wizard page, the ontology modeling workbench, and collapsible nav groups) ** + Current release: **v1.11.3** — **MS4 annotation loop (a bypass-mode labeling supply chain, zero hot-path impact): Label Studio CE 1.13.1 as a transient compose-profile workspace (AL's Lance ADL is the source of truth); ontology template → LS labeling-config generator (entities/relations + L4 five sections, manual-XML override); a dependency-free LSClient (legacy Token auth with PAT-refresh fallback, explicit zero-proxy intranet dialing); annotation project registry (V018/V019 watermark); four-strategy sampling (uncertainty/diversity/failure_case/committee, largest-remainder quotas with source-missing redistribution); pre-annotation = real hyper-extract runs embedded as static LS predictions (no ML backend); masking before dispatch (L2 generalization + L3 HMAC pseudonyms, fail-closed, extraction runs on masked text so spans stay aligned); one-click dispatch (202 background); recovery via full export reconciliation with content-hash adl_id idempotency, per-(row,annotator) version chains, watermark that only advances on annotated tasks, Fleiss' kappa + signature-based adjudication (discordant rows auto-generate arbitration tasks; ground-truth marks close the loop), a 30s recover scheduler with circuit breaker, and a console annotation page; the W4 fan-out review (13 findings) and W5 live calibration (6 findings — stable row ids, 404-only LS rebind, async worker loop-affinity, require_masking gate, webhook secret, scoped MinIO credentials) were all cleared same-day; the pilot ran the full loop against real Bailian + real LS with kappa/adjudration/version-chain/audit DoD assertions) ** + Current release: **v1.11.2** — **MS3 decision & action layer (predicate DSL with a hand-written tokenizer and frozen-AST evaluation, action catalog + scenario version chains in V016/V017, an 8-step execution middleware with idempotency-before-preconditions, decisions/assess API, console pages, and a vertical-slice e2e)** + Current release: **v1.11.1** — **MS2 semantic layer (object identity + semantic alignment + Object Set queries): DR15 D-1/D-2 model extensions — contracts gain column `label`, table-section `lifecycle`, reference `cardinality`+`kind` (registration-only, compiler output provably unchanged) and `ontology_rules` gains five-way `rule_type` + independent `version` (V013); F2.1 object identity (contract-pattern identifier parsing + V014 entity_map, source-id→object-id); F2.2 semantic alignment (affine unit registry with temperature offsets and %LEL fenced as its own dimension; closed-kind configs in V015 with lineage events; query-view SQL projections); F2.3 `POST /api/v1/objects/query` returns business objects (identifier components, label/unit attributes, lifecycle state, `_links` with cardinality/kind, optional KG neighbors, active-rule references) via server-composed restricted SQL on the exact /query/olap security path; F2.4 console objects.html object browser; W1–W4 three-way review (0 critical) cleared same-day; the cross-source same-caliber DoD locked as an automated e2e (same physical quantity through two source systems → same object, same attribute, same unit)** + Current release: **v1.11.0.3** — **hardening batch (contract enforce pilot + homepage-401 root fix + six review-P2 items): per-dataset quality-gate mode overrides (`GATE_MODE_OVERRIDES` JSON blob — demo_gas switched to enforce after baseline gating at 0.00% <10% with zero skipped sections, contract-rejected dead-letter rows now carrying specific violation kinds); the browser favicon 401 fixed at the root (public path + asset + long cache — the true source of the "intermittent homepage 401" probe); six P2 items from the v1.11.0.1 review cleared (container-table delete now reclaims its registry row + `DELETE ?table=`, container probes no longer mkdir garbage directories, container-level delete mutually exclusive with per-table write locks, Gravitino per-table 4xx no longer aborts sibling tables, contract pattern inner regexes try-compiled at save with a length cap, and baseline NULL samples render as explicit `<NULL>`)** + **v1.11.0.2** — **dataset contracts & multi-table containers (DR13/DR14): a contract package compiling per-table column rules into DuckDB predicates, wired into the quality gate as a fourth stage (shadow/enforce, evaluation failures fail-closed in enforce); SQL/ClickHouse/Kafka/Iceberg/Delta sources all gated; contracts admin API + console page + offline baseline script with --max-rows guardrails; multi-table container datasets with two-part addressing via `?table=` (the {name} route pattern keeps rejecting dots), Gravitino dataset→schema mapping, container-aware backup/restore, and console container views — plus the pre-release four-dimension review (9 P0 + 10 P1) cleared the same day: table-level deny-read closed end-to-end, pooled DuckDB sessions cleaned after every query (cross-user registration read blocked), and gate fail-open paths removed** + **v1.11.0** — **MS1 ontology & rules foundation (implicit template ontology formalized into machine-checkable SHACL — an `ontology:` section on templates, flagship `project_concept_graph` carrying 22 entity classes / 16 relations / 72 type-pairs kept in lockstep with the runtime whitelist; an ontology gate at KG-build finish in shadow mode with reject/warn grading, timeout fail-closed, `arrow_lake_ontology_check_total` metrics and violation summaries on build status; version-chain snapshots with structured diffs in system_db V010 plus an auditable rules registry; admin API `/api/v1/ontology` and a console page; ontology→guideline rules rendering so extraction prompt and validator share one source of truth; defect-escape baseline measured at 29.21% on the flagship dataset — hence enforce stays off)** + **v1.10.8** — **v1.10.8 MS1-prereq hardening (all 10 sync ingest endpoints moved onto the dedicated ingest thread pool so heavy ingest can no longer starve auth IO; control-plane failures are now fail-closed — a row/column-ACL store outage returns 503 instead of silently running queries unfiltered, and a tva-provider outage rejects tokens unless `auth_tva_fail_open` explicitly opts out; dead-letter tables moved into the internal `_{ds}_dead_letter` namespace with admin-only access and case-insensitive guarding for both new and legacy spellings; `PUT /deny/{ds}` finally binds the write path — all 21 ingest/index/upload/delete endpoints enforce deny(write), with write-scoped personal tokens honored but never able to bypass a deny; auth timeouts surface as 503 instead of 401/500 so transient storage hiccups don't force re-login)** + **v1.10.7 P1 data-plane hardening (source-level dataset ACL enforcement via sqlglot — row filters pushed into the query and hidden-column references rejected at AST level, closing the alias/aggregation bypass class; dataset deny/ACL guards on every read endpoint; RAG fail-closed on ACL-restricted datasets; ingest work isolated onto its own thread pool; auth hot path off the event loop; SIGTERM no longer hijacked so graceful shutdown actually runs; the ingestion quality gate wired in shadow mode with its three latent bugs fixed; pool/tag-ACL/embed-guard reliability fixes)**, plus the post-release four-dimension review hardening folded into v1.10.7 (case-insensitive identifier matching in SQL ACL enforcement — the CRITICAL bypass — plus COLUMNS()-wildcard rejection, scope-aware alias resolution, quoted row-filter subqueries for `--` names, the all-rejected-batch gate fix, and fail-open tag-ACL recovery closures) + **v1.10.6 authentication & authorization hardening (JWT `aud` claim + per-user token invalidation, admin-issued one-time password reset, full admin write-op audit trail + shared-API-key deprecation nudge, JWKS endpoint + RS256 recommendation, and scope-based authorization via `require_permission` with role fallback — the four seams a future external IdP needs, without adding one)** + v1.10.0 knowledge extraction template management (M1–M5: dynamic loading, CRUD, binding, AI authoring, dry-run, quality validation harness) + v1.10.1 stability & governance hardening (docling GPU triton JIT fix, KG template fallback path, config consolidation, examples→cookbook) + **v1.10.2 text-document incremental build (ingest/KA/KG incremental), performance benchmark suite expansion, and timeout/reliability hardening (OLAP `conn.interrupt()` watchdog so a stuck scan can't strand the pool, async-task heartbeat + orphan reaper so worker death can't strand a task in `running`, and per-call timeouts across the ingest chain), with an SSD performance re-baseline** + v1.10.3 docling throughput & quality (ThreadedPdfPipelineOptions page-batching to saturate GPU, RapidOCR, confidence-gated OCR retry, page-image export for ColPali/CLIP multimodal RAG, baked bge-m3 tokenizer for offline HybridChunker) + **v1.10.4 per-dataset native lance scan opt-in + D-state circuit breaker (large vector-free datasets 34–145× faster via Rust aggregation pushdown; breaker auto-degrades to pyarrow on repeated D-state), OLAP result pagination + per-field distribution stats, multi-statement SQL guard, and structlog level filtering / gravitino sync log-noise reduction**. See [CHANGELOG](CHANGELOG.md) for the full history and roadmap direction (deeper multimodal, distributed scale-out, more extraction backends).
