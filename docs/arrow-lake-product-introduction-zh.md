...
文档包含 19 章双语 Cookbook（英文和中文），50+ 个可运行示例，覆盖从基础摄入到高级 GraphRAG、数据脱敏、血缘可视化的每个功能。权威的 [`ARCHITECTURE.md`](./architecture-design/ARCHITECTURE.md)（17 章 + 8 图集 + 附录 A–E，主架构参考）以及 [`cookbook/`](./cookbook/) 实战手册，提供更深层的架构上下文、配置参考和部署流程。
