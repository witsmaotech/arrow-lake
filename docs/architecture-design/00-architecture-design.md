...
> 📐 **图集速览**：本文以图为骨架（Midnight Blueprint 8 套图）重组架构要点。**权威文本参考**见 [`ARCHITECTURE.md`](ARCHITECTURE.md)（逐项核实源码，深度 API 回链）；本文深度细节回链其 §3/§5。

> **版本基线**：v1.11.0.2（[`arrow_lake/_version.py`](../../arrow_lake/_version.py) = `pyproject.toml` = 1.11.0）
> **文档日期**：2026-08-03
...
Docker Compose 单栈交付（镜像 `arrow-lake:1.11.0.2`），六个功能区：
