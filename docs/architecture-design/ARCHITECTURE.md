...
> **版本基线**：v1.11.6.6（已合并 `master`；`arrow_lake/_version.py` = `pyproject.toml` = 1.11.6.5）
> **文档日期**：2026-08-03
...
| **v1.11.3** | 2026-08-30 | **MS4 标注闭环(annotation loop,旁路红线零热路径)**:LS CE 1.13.1 独立 compose profile(transient 非 SoT);本体模板→LS config 生成器(实体/关系/L4 五段,手写 XML 覆盖);LSClient(urllib 零依赖,legacy Token 直连+PAT refresh 回退,内网零代理直连);项目注册 CRUD(V018+watermark V019);四策略采样(uncertainty/diversity/failure_case/committee,最大余数法配额+源缺失重分配);预标注=HE 真抽取→LS 内嵌 predictions(S4 静态,无 ML backend);脱敏前置(L2 泛化 regex+L3 HMAC 假名,fail-closed;HE 在脱敏文本上抽取 span 自洽);dispatch 一键链(202 后台,主 loop async 保 extractor loop 亲和);recover=export 全量对账(列表视图裁剪 annotations,live 实证)+watermark 仅随带标注任务推进+adl_id=sha1 内容幂等+(row,annotator) 版本链递增;Fleiss' kappa+signature 仲裁状态机(分歧→仲裁 task,ground_truth 免检 approved 闭环);30s scheduler(连续 5 轮失败熔断);webhook 加速通道(_PUBLIC_PATHS+共享密钥);console annotation.html(D3 独立导航组);fan-out review 13 项+W5 live 校准 6 项(row_id 稳定化/404-only 重建/require_masking/regex 帽/scoped MinIO 凭据)当日全清;试点 demo_annotation_alerts 真百炼×真 LS 全流程 DoD 断言(脱敏/预标注 spans/双标注者/kappa/仲裁 task 9 个/版本链 v1-v4/审计) |
| **v1.11.4** | 2026-08-31 | **MS5 五维质量门+发布层(Master Plan 收官)**:五维评估引擎(契约 `quality:` 节权重/星级/bronze-silver-gold 准入/逐维一票否决与诚实降级);相关性回路(LS 三分类+LLM 直评降级+反哺清单);漂移监控(KL 散度 32 桶+发布时基线快照);发布门(准入+否决+拒绝劣化+漂移;force 留审计;Lance 版本锁定+语义化 tag+datasheet YAML;三读同源);语料四形态(SFT=ADL 五段/预训练=KG 快照/RLHF=decisions_history×ADL 配对/黄金集;红线④脱敏 fail-closed);数据飞轮(低置信 auto 回流);F1.7 评审材料包;同日四维综合 review(1C/15H/22M)五批全清(C1 双标注回收治本=V024 复合 watermark+webhook 真触发;H2 Redis 回收互斥;H9 drift 向量化+event-loop 解冻;H8 行动写唯一性核验;M2 治理审计九端点;M18 332 测试解封);全站 console 23 页 64 项交互逐一实测(8 处'从未调通'级 API 调用修复);生产向导页 hq-guide+本体建模工作台+导航六组折叠;identity naive-expires 修复;试点 demo_annotation_alerts 补契约全链 force 发布 v1.4.0(sft 10/golden 8) |
| **v1.11.6** | 2026-09-08 | **字段与多模态类型扩展**:前置批=字段变更前后端联动(schema/migrate dry_run 方言 lint:TRIM/SUBSTR/双引号字面量/CASE 四坑静态拦截 + 表达式 5 行真数据采样试算 + console dataset-detail「字段变更」三操作面板,容器表 ?table= 寻址);主体=多模态字段类型扩展(`resolve_lance_type` 全语法:`vector:<dim>`/`blob`(lance.blob.v2 大对象)/`timestamp`/`date32`/`large_*`;加列双模式——SQL 表达式 ↔ 类型化占位列(new_type+空 expr→`add_null_column` 全 NULL 落库后回填,向量端到端 live 验证:占位→embed 空值回填→语义检索命中);blob 版本墙 dry_run 预检+apply 双保险(lancedb 0.36 默认写 2.1→存量表拒,给重写 2.2/退 binary 双路指引);向量维度检查从死代码转正;string→binary 语义防护;未知类型全路径 fail-closed;Lance 11.0 多模态实证入册(blob API=blob_field/blob_array、占位列四类型通路、SQL 产不了向量列、alter cast 面、update dict 形态)) |
| **v1.11.6.1** | 2026-09-10 | **Docling VLM 档可用性修复+标题层级恢复**(补丁批):HF_HOME 由镜像只读 /opt/hf-cache 改指 docling-cache 可写卷(standard 两模型与镜像 bake 同 commit 入卷 + granite-docling-258M 12 文件经 **ModelScope 直连**预置入卷——HF/代理双断时国内镜像同 repo 同 sha256 可达,构造标准 HF cache 布局 refs/snapshots;HF_HUB_OFFLINE 维持离线);`docling_pipeline_type=vlm` 从空接线转真实可用(live 实测:granite GPU 转换 markdown 结构正确,峰值显存 +1.3GB,init 8.6s/单页 53.9s);F1 标题层级恢复(`docling_heading_hierarchy` config 默认开,HeadingHierarchyOptions 书签/编号/字体样式三信号 + generate_parsed_pages,零模型开销——PDF 标题从扁平 level-1 恢复层级,markdown 深度/分块 heading path/RAG 结构信号受益;converter 签名含开关);dev override HF 布线与生产对齐(移除 HF_HUB_CACHE host 直挂)。登记轨:OCR 上 GPU(卷解除 read-only 后 torch 后端可行)/VLM vLLM 服务化/Flash Attention2/NativePdfPipeline/图片描述(可走 ollama API 型)/视频 ASR+说话人分离 |
| **v1.11.6.2** | 2026-09-11 | **RapidOCR OCR 上 GPU**(补丁批,P0-2 落地):镜像内 onnxruntime(CPU 1.29)→ **onnxruntime-gpu 1.30(CUDA 13 构建)**——与 torch cu130 同代,动态库零新增(compose 一条 `LD_LIBRARY_PATH` 复用 torch 自带 `nvidia/cu13/lib` + `nvidia/cudnn/lib`;路径硬绑镜像 python3.12);模型零改动(rapidocr 3.x 包内 PP-OCRv6 det/rec + v4 cls ONNX 继续用);**项目代码零改动**——docling 按 `AcceleratorOptions(CUDA)` 自动设 `use_cuda`(GPU_COUNT=0 判 AUTO/CPU 自动回落 CPU EP);Dockerfile 换包步骤走清华镜像直连 + `--no-deps`(flatbuffers/numpy 已在依赖树);解锁 page_batch 上调空间(OCR 张量 RAM→VRAM,当年 page_batch=64 OOM 的根因缓解;api 内存限制现为 32GiB) |
| **v1.11.6.3** | 2026-09-14 | **Docling VLM 档 API 化**(补丁批):`docling_vlm_endpoint/model/api_key/concurrency` 四 config 字段——VLM 档可指向任意 OpenAI 兼容端点(`ApiVlmEngineOptions` + `enable_remote_services=True`,云 API 或自托管推理服务);inline 本地 Transformers 仍为默认;converter 签名按 endpoint+model 分桶(缓存正确性);compose 透传 `DOCLING_VLM_*` 五键(.env 单一控制面)。**架构定调:平台不自带推理服务**(定位=数据湖仓,非大模型原生应用;vLLM 容器方案评估后弃——9GB 镜像国内 mirror 大层 EOF 拉取失败 + 重组件与定位不符)。live 验证(百炼 qwen-vl-max + qwen preset markdown 输出):扫描件单页 **5.2s vs inline 53.9s(10x)**、零本地显存、数字原生 PDF 内容完整;坑=百炼 `qwen-vl-max-latest` 变体名 403(用 `qwen-vl-max`)、VLM API 失败时 docling 仍返 SUCCESS 空 content(需看 errors) |
| **v1.11.6.5** | 2026-09-14 | **图片描述 enrichment**(补丁批,RAG 文本路线):`docling_picture_description` + endpoint/model/prompt 四字段(默认关,API 型 OpenAI 兼容,key 复用 VLM 字段)——文档内图片获中文描述段落(`<!-- image -->` + 文本),随 `export_to_markdown` 自动进 `parsed.text` → chunk/embed/FTS 全链可查,**下游零改动**,与 ColPali 向量路线互补;standard/vlm 两档汇合点共用接线,endpoint/model 缺失 warning 降级不炸转换;converter 签名含开关+端点元组。live(百炼 qwen-vl-max):合成柱状图 13.0s,描述数值全对(甲烷70/丙烷50/丁烷30)直进 text;负例双验(默认关零回归/缺配置降级)。坑=standard 档 API 型 enrichment 同须 enable_remote_services;docling-core 图片导出是注释占位+文本段(非 ![]() 形态) |
| **v1.11.6.5** | 2026-09-14 | **PII 分级自动建议**(补丁批,搁置 W2 #3 复活;与登记轨「docling PII」合并裁定——按产品锚准则裁掉 GLiNER 模型面,交付确定性建议引擎):`GET /datasets/{name}/classification/suggest`(EDITOR 只读零写入,不依赖 classification store)——`arrow_lake/quality/pii_suggest.py` 零模型内容扫描(身份证 GB11643 校验位→restricted;银行卡 Luhn 标注启发式/手机号/车牌/详细地址→confidential;邮箱/固话/列名弱语义→internal;零证据→public)+ 列名语义(坐标/地址区划/联系人;org/unit/uid/url 负面清单防 org_name 误报)+ 自由文本列提示;档位聚合有量门槛(内容命中 ≥3 行或 ≥1%,列名证据恒有效);**大表安全**:Lance `scanner(字符串列, limit=500)` 存储层截断(107M 行 ontime 0.7s 无 OOM);console 分级面板「建议→一键采纳」(采纳走同一 PUT+审计路径);红线=建议≠自动写入(登记不校验原则不变)。live DoD:lpg_danger→confidential(longitude/latitude+district/street 列名证据,887ms);踩坑=追踪类系统 id 列撞银行卡 Luhn(lpg_danger.uid 2026 开头长数字 45/500 命中,~9% 正是 Luhn 偶然命中率)→内容扫描跳过负面清单列 |
| **v1.11.6.6** | 2026-09-14 | **四维 review 加固批**(范围 a36867a..HEAD 六版本 +5500 行,3 HIGH+12 MEDIUM+12 LOW;规划 docs_offline/v1.11.6.6-hardening-plan.md):**H-1 场景孤儿回收年龄门槛**——V027 `scenario_instances.updated_at` 列 + `mark_orphaned_running(stale_seconds=180)` 年龄条件 + runner 20s 心跳(`touch`)维持锚点,sibling worker 重启(panic/OOM)不再无条件杀活 runner;**H-2 版本钉版**——resume 取 `get_version(scenario_id, version=实例行 scenario_version)`,middleware 归属校验同钉(`execute_action(scenario_version=)`),场景升 v2 后旧实例续跑不再被新版 step 集误拒/混入;**H-3 列级 ACL 两旁路面封堵**——`deps.caller_visible_columns()` 共享 helper(ADMIN 旁路/小写匹配/二段名分层查表):PII suggest 按调用者 `visible_columns` 交集(受限视角零列采样不回落全列)+ note 声明未施加行级过滤,场景实例列表剥 `context_json`/详情按调用者裁剪 `context.target`(无读权置空+acl_pruned 标记);M1 drop_index 后 `invalidate_async_table`;M2 dry-run preview 过 `_validate_sql_expr` 危险关键字黑名单;M3 PII 单值截断 4096(实测 email 无界正则 O(n²) 回退 500 行 15s→RFC 5321 上界线性化亚秒);M4 docling page_batch 赋值移 standard/VLM 汇合点(VLM 档此前被 docling 默认 4 封顶并发);M5 parse 缓存 key 加 docling 签名(描述开关翻转不再复用旧解析);M6 图片描述并发/超时 config 化(compose 透传);M7 AND 批补偿待办聚合;M8 补偿核销端点 `POST .../compensation/{step}/ack`+console 核销按钮;M9 instantiate 同对象 running 查重 409 + resume CAS(`resume_instance` 条件 UPDATE,并发仅一活);M10 网关降权口径收敛(因子只作用 runner 上下文,中间件服务端重评独立,by design);M11 契约 identifier/person 字段语义→LOW 证据 + console suggest 死按钮修复(handler 绑定移出 loadPii);M12 实例列表 offset 翻页+COUNT 真 total;LOW:lint 剥单引号字面量/`isdecimal` 维度/terminate CAS 不覆写终态/onnxruntime providers 首建日志/decisions docstring 修正/PII 面板 esc 补齐/门面归一化+drop 回落 mock 测试 |
| **v1.11.5** | 2026-09-05 | **投产准备+场景执行引擎(MS3 转正)+真实数据首跑(发版闸门)**:W1 测试隔离治理(conftest 四锚点 storage/api-key/rate-limit/redis 根除「全量 448 随机失败」类,448 failed→2 第三方/64min 稳定;产线真缺陷 11 个当日清)+CI 骨架三 job(lint E9/F821 门禁/全量单测/runtime-ci 镜像 build+容器冒烟);W3 场景执行引擎(`actions/runner.py`:XOR 网关在引用步 requires 处求值+落选臂 skipped 级联/AND gather 并发/死锁守卫/deadline 超时升级/声明式补偿挂人工待办/terminate/断点续跑尊重幂等 already_in_effect)+V025 场景实例两表+五端点(instantiate 202/instances 列表详情/terminate ADMIN/resume EDITOR,注册序 instances 先于 {id})+console actions.html 执行态(行内试跑/实例抽屉/补偿待办);W2 PII 分级(V026 四档登记不校验,`/datasets/{name}/classification`+变更审计+console 分级面板)+corpus 导出分级门禁(未分级 422 须显式豁免,审计 corpus.unclassified;confidential/restricted 未脱敏豁免提示点名档位);W4 confidence 真实语义(未命中规则 0.5 基线/每条 unruly −0.1 封顶 −0.3/substitute 臂 ×0.9,发布门阈值零改动);W2 #6 真实数据首跑硬 DoD 严格达成:lpg_danger 2 万行(合肥液化气隐患)摄入→契约 v2(uid identifier/danger_state lifecycle/quality.weights 按域调权 diversity 归零)→confidential→4 active 规则→双标注 κ=1.0→五维 98.69/gold→v1.1.0 干净发布不走 force(v1.0.0 force 留档);首跑当日清 4 真缺陷(标注采样稀疏文本列饥饿 8×cap 扩窗重扫/cap 截断先于过滤/LS 相关性词表须平台标准三分类/预标注按项目 labeling config 过滤防悬空 relation 崩 LS 页) |
| **v1.11.2** | 2026-08-28 | **MS3 决策与行动层**:W1 语言地基(谓词 DSL 手写 tokenizer+优先级爬升+frozen AST 求值,无 eval,512 帽,unicode 路径含中文,缺失 path→比较恒 False,lru 缓存与 condition_expr 同源;模板封闭两形态 {{path}}/{{now()}};Action/Scenario pydantic 模型+保存期校验——effect 封闭集/xor 强制 else/ISO-8601 duration/目录引用完整);W2 存储(V016 actions_catalog 版本链+idempotency_keys owner-token 裁决,V017 scenarios,/api/v1/actions ADMIN CRUD+execute,console actions.html);W3 研判引擎(Object Set 编排段提取为共享 fetch_object_rows,deny/ACL 闭包与 /query/olap 字面同源,零回归硬验收含 4 RBAC 接线审计;POST /decisions/assess VIEWER 对齐后口径,unruly fail-open 到条,confidence 恒 1.0,actionable 目录反查;黄金集 5 对象×7 规则);W4 执行中间件八步序(幂等④先于前置⑤——自失效型行动重放兑现已生效;update_lifecycle=storage 原生行级 update 扩 table= 容器寻址 D1-①;notify=user_state;进程内 pub/sub 异常隔离;console decisions.html 研判台=结构化引用链 F3.2 轻实现);两路 fan-out review 13 findings 当日清偿(写向门禁 H-1/前置槽泄漏 H-2/assess 服务端重评防伪造 H-3/审计兜底/超时禁自动重放/to_state 词表);W5 vertical slice e2e(告警→研判→gate→发布断言 lifecycle 真写+审计归属+事件+通知→幂等重放→越权 403)+三度量基线脚本 |
| **v1.11.1** | 2026-08-27 | **MS2 语义层(对象标识/语义对齐/Object Set)**:DR15 D-1/D-2 模型扩展——契约列 `label`/表节 `lifecycle`/引用 `cardinality`+`kind`(登记不校验,compiler 零改动测试钉住)+ontology_rules 五分类 `rule_type`+独立 `version`(V013);F2.1 对象标识(契约 pattern 解析 `semantic/identity.py`+V014 entity_map 源系统 ID→对象 ID,ADMIN 显式维护);F2.2 语义对齐(`semantic/units.py` 仿射单位注册表含温度偏移、%LEL 独立维度;封闭种类 alignment 配置 V015+lineage 事件;projection_sql 查询视图投影);F2.3 `POST /api/v1/objects/query` 业务对象聚合(标识组件双路径/label+unit 属性/lifecycle 状态/带基数 `_links`/可选 KG 邻居封顶/active 规则引用),服务端拼装受限 SQL 走 /query/olap 同一条安全路径(deny 双查+enforce_sql_acl+接线审计测试);F2.4 console objects.html 对象浏览页(类型树/详情抽屉/关联跳转);W1-W4 三路 review(0 critical)当日清偿 14 项(types 读权/entity 批查 N+1/int64 精度/like-422/quoted FROM/VIEWER 剥离);跨源同口径 DoD 固化自动化 e2e(真 Lake+真 DuckDB)。 |
| **v1.11.0.3** | 2026-08-27 | **加固批（契约 enforce 试点 + 首页 401 根治 + review P2 六项）**：per-dataset 质量门禁档覆盖（`GATE_MODE_OVERRIDES` JSON blob——全局保持 shadow、单数据集切 enforce；demo_gas 经基线门控 0.00%/<10% + 零 skipped 切 enforce，契约拒收行死信带具体 violation 明细）；浏览器 favicon 401 根治（根路径公开 + 资产 + 长缓存——"首页间歇性 401"的正源，与 token 无关）；review §三 P2 六项：容器表删除回收 registry 行 + `DELETE ?table=`、`list_container_tables` 探测不再 mkdir 垃圾目录、容器级删除与表级写锁互斥（按序获取表锁再删）、Gravitino per-table 4xx 不再中断同容器其余表、契约 pattern 内层 regex 保存时试编译 + 256 长度帽、基线 NULL 样本显式 `<NULL>`。D3 校准：registry=镜像（回收闭环），读接通留 MS2。 |
| **v1.11.0.2** | 2026-08-27 | **容器面补全（发版后目测验收反馈批）**：tidy/data-prep 两页容器表支持（`?table=` 全链接线：facade read/restore/add_columns_table/quality_filter/deduplicate/llm_enrich + quality 七端点 + clean）；**表级 deny 双机制闭环**（deny_action 列表 + DatasetACL.denied_actions 双查——首版只查 ACL 对象，admin 建的表级 deny 在 `?table=` guard 上从不生效）；schema/migrate 容器表支持（add/alter/drop_column 接 table，`_open_lance` 容器模式 container=/table= kwargs 修正）；**lance SQL 方言预存 bug 修**（add_columns 表达式双引号=字符串字面量、TRIM/SUBSTR 不支持、regexp_replace 需 'g' flag——TIDY 菜单重写）；dataset-detail 初始加载零 4xx；存量 21 个 quality 测试腐烂修复（fixture 角色对齐）。
