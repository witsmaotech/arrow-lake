...
> 想直接看 GraphRAG 问答？见 [08 RAG](./08-rag-pipeline-zh.md) 的 GraphRAG 段与本章 §9。下面先讲如何**构建**一张可被 GraphRAG 使用的知识图谱。
...
## 📚 附录：v1.7–v1.10 KG 演进史（版本记录，可选阅读）

> 本节为 KG 子系统的版本演进记录（抽取后端 / doc_type 路由 / per-dataset / 增量 / 质量 / 模板管理）。**新手可跳过本节** —— 核心用法见上文 §1–§11；完整变更见 [CHANGELOG](../../CHANGELOG.md)。保留此附录供老用户与贡献者了解设计背景。
